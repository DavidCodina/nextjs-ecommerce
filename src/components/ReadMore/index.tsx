...
export const ReadMore = ({
      ...
      >
        {!open ? (
          <span className='flex items-center gap-[0.25em]'>
            {moreText} <CircleChevronDown className='size-[1.25em]' />
          </span>
        ) : (
          <span className='flex items-center gap-[0.25em]'>
            {lessText} <CircleChevronUp className='size-[1.25em]' />
          </span>
        )}
      </button>
    )
  ...
  return (
    <div {...otherProps} className={cn('read-more-container', className)}>
      <div
